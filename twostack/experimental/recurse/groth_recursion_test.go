...
func TestGrothOuterProof(t *testing.T) {
	...
	innerCcs, innerVK, innerWitness, innerProof, err := computeGrothInnerProof(t, InnerCurve.ScalarField())

	//now verify the outer proof
	pubWitness, err := (*innerWitness).Public()
	assert.NoError(err)
	genesisWitness, err := groth16.ValueOfWitness[ScalarField](pubWitness)
	assert.NoError(err)
...
func computeGrothInnerProof(t *testing.T, innerField *big.Int) (*constraint.ConstraintSystem, *native_groth16.VerifyingKey, *witness.Witness, *native_groth16.Proof, error) {
	...
	return &innerCcs, &innerVK, &innerWitness, &innerProof, nil
}
